(() => {
    ...
    function calculatorStageFilling() {
        ...
        function checkIsFillingStage(stageInputs, stages, index) {
            ...
            const isFillingStage = stageInputs.every((input => {
                const isNumberFloatFormat = input.hasAttribute("data-number-float-format");
                if (!isNumberFloatFormat) return input.value !== ""; else {
                    const value = input.value;
                    const isValidDecimal = /^(\d+[.,]?\d*|\d*[.,]?\d+)$/g.test(value);
    ...
    function calculateDiminishedValue() {
        ...
        if (preAccidentInput && damageInput && mileage && resultElem) [ preAccidentInput, damageInput, mileage ].forEach((input => {
            input.addEventListener("input", (e => {
                const preAccidentValue = parseFloat(preAccidentInput.value.replace(",", "."));
                const damageValue = parseFloat(damageInput.value.replace(",", "."));
                const mileageValue = parseFloat(mileage.value.replace(",", "."));
