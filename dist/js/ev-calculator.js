 ...
 */            (function(global, factory) {
                ...
                numeral = function(input) {
                    ...
                    if (numeral.isNumeral(input)) value = input.value(); else if (input === 0 || typeof input === "undefined") value = 0; else if (input === null || _.isNaN(input)) value = null; else if (typeof input === "string") if (options.zeroFormat && input === options.zeroFormat) value = 0; else if (options.nullFormat && input === options.nullFormat || !input.replace(/[^0-9]+/g, "").length) value = null; else {
                        for (kind in formats) {
                ...
                numeral._ = _ = {
                    numberToFormat: function(value, format, roundingFunction) {
                        var abbrForce, abs, int, precision, signed, thousands, output, locale = locales[numeral.options.currentLocale], negP = false, optDec = false, leadingCount = 0, abbr = "", trillion = 1e12, billion = 1e9, million = 1e6, thousand = 1e3, decimal = "", neg = false;
                        value = value || 0;
                        abs = Math.abs(value);
                        ...
                        if (precision) {
                            ...
                            int = decimal.split(".")[0];
                            if (numeral._.includes(decimal, ".")) decimal = locale.delimiters.decimal + decimal.split(".")[1]; else decimal = "";
                        ...
                        if (negP) output = (negP && neg ? "(" : "") + output + (negP && neg ? ")" : ""); else if (signed >= 0) output = signed === 0 ? (neg ? "-" : "+") + output : output + (neg ? "-" : "+"); else if (neg) output = "-" + output;
                        return output;
                    },
                    stringToNumber: function(string) {
                        var abbreviation, value, regexp, locale = locales[options.currentLocale], stringOriginal = string, abbreviations = {
                            thousand: 3,
                            million: 6,
                            billion: 9,
                            trillion: 12
                        };
                        if (options.zeroFormat && string === options.zeroFormat) value = 0; else if (options.nullFormat && string === options.nullFormat || !string.replace(/[^0-9]+/g, "").length) value = null; else {
                            value = 1;
                    ...
                    reduce: function(array, callback) {
                        ...
                        var value, t = Object(array), len = t.length >>> 0, k = 0;
                    ...
                    toFixed: function(value, maxDecimals, roundingFunction, optionals) {
                        var boundedPrecision, optionalsRegExp, power, output, splitValue = value.toString().split("."), minDecimals = maxDecimals - (optionals || 0);
                        if (splitValue.length === 2) boundedPrecision = Math.min(Math.max(splitValue[1].length, minDecimals), maxDecimals); else boundedPrecision = minDecimals;
                ...
                numeral.validate = function(val, culture) {
                    ...
                    if (localeData.delimiters.thousands === ".") _thousandSep = "\\."; else _thousandSep = localeData.delimiters.thousands;
                    temp = val.match(/^[^\d]+/);
                    if (temp !== null) {
                        val = val.substr(1);
                ...
                numeral.fn = Numeral.prototype = {
                    ...
                    format: function(inputString, roundingFunction) {
                        var kind, output, formatFunction, value = this._value, format = inputString || options.defaultFormat;
                        roundingFunction = roundingFunction || Math.round;
                        if (value === 0 && options.zeroFormat !== null) output = options.zeroFormat; else if (value === null && options.nullFormat !== null) output = options.nullFormat; else {
                ...
                (function() {
                    numeral.register("format", "bps", {
                        ...
                        format: function(value, format, roundingFunction) {
                            var output, space = numeral._.includes(format, " BPS") ? " " : "";
                            value *= 1e4;
                ...
                (function() {
                    ...
                    numeral.register("format", "bytes", {
                        ...
                        format: function(value, format, roundingFunction) {
                            var output, power, min, max, bytes = numeral._.includes(format, "ib") ? binary : decimal, suffix = numeral._.includes(format, " b") || numeral._.includes(format, " ib") ? " " : "";
                        ...
                        unformat: function(string) {
                            var power, bytesMultiplier, value = numeral._.stringToNumber(string);
                            if (value) {
                ...
                (function() {
                    numeral.register("format", "currency", {
                        ...
                        format: function(value, format, roundingFunction) {
                            var output, symbol, i, locale = numeral.locales[numeral.options.currentLocale], symbols = {
                ...
                (function() {
                    numeral.register("format", "ordinal", {
                        ...
                        format: function(value, format, roundingFunction) {
                            var output, locale = numeral.locales[numeral.options.currentLocale], ordinal = numeral._.includes(format, " o") ? " " : "";
                ...
                (function() {
                    numeral.register("format", "percentage", {
                        ...
                        format: function(value, format, roundingFunction) {
                            var output, space = numeral._.includes(format, " %") ? " " : "";
                            if (numeral.options.scalePercentBy100) value *= 100;
                        ...
                        unformat: function(string) {
                            ...
                            if (numeral.options.scalePercentBy100) return number * .01;
                            return number;
                        }
                    });
                })();
                    ...
                    numeral.register("format", "time", {
                        ...
                        unformat: function(string) {
                            ...
                            if (timeArray.length === 3) {
                                seconds += Number(timeArray[0]) * 60 * 60;
    ...
    (() => {
        ...
        function checkboxInput() {
            ...
            if (checkbox) window.addEventListener("click", (e => {
                const target = e.target;
                if (target.classList.contains("checkbox__btn")) {
                    const input = target.parentElement.querySelector(".checkbox__input");
        ...
        function showInputPrefix() {
            const inputs = document.querySelectorAll("[data-input-prefix]");
            if (inputs.length) inputs.forEach((input => {
                input.addEventListener("input", (() => isShowPrefix(input)));
                isShowPrefix(input);
                const form = input.form;
                if (form) form.addEventListener("reset", (e => {
                    setTimeout((() => {
                        isShowPrefix(input);
                    }), 0);
                }));
            }));
            function isShowPrefix(input) {
                const inputParentElem = input.parentElement;
                const prefixSymbol = inputParentElem.querySelector("[data-symbol-prefix]");
                if (prefixSymbol) input.value !== "" ? prefixSymbol.classList.add("_show-prefix") : prefixSymbol.classList.remove("_show-prefix");
            }
        }
        function calculatorStageFilling() {
            let stages = document.querySelectorAll("[data-stage]");
            if (stages.length) stages.forEach(((stage, index) => {
                let stageInputs = Array.from(stage.querySelectorAll("input,select"));
                if (stageInputs.length) {
                    stageInputs.forEach((input => {
                        [ "input", "change" ].forEach((event => {
                            input.addEventListener(event, (() => checkIsFillingStage(stageInputs, stages, index)));
                        }));
                    }));
                    checkIsFillingStage(stageInputs, stages, index);
                }
            }));
            function checkIsFillingStage(stageInputs, stages, index) {
                const nextStages = Array.from(stages).slice(index + 1);
                const nextStage = stages[index + 1];
                const isFillingStage = stageInputs.every((input => input.value !== ""));
                if (nextStage) {
                    if (isFillingStage) {
                        nextStage.classList.remove("_disabled");
                        stageInputs = Array.from(nextStage.querySelectorAll("input,select"));
                        checkIsFillingStage(stageInputs, stages, index + 1);
                    } else nextStages.forEach((stage => {
                        stage.classList.add("_disabled");
                        enableDisableInputsInStage(stage, isFillingStage);
                    }));
                    enableDisableInputsInStage(nextStage, isFillingStage);
                }
            }
            function enableDisableInputsInStage(stage, isFillingStage) {
                const inputsInStage = stage && stage.querySelectorAll("input,select,textarea,a,button");
                inputsInStage && inputsInStage.length && inputsInStage.forEach((input => {
                    isFillingStage ? input.removeAttribute("tabindex") : input.setAttribute("tabindex", "-1");
                }));
            }
        }
            ...
            if (inputs.length) inputs.forEach((input => {
                input.addEventListener("input", (() => {
                    if (input.value !== "") {
                        ...
                        const formattedValue = numeral(value).divide(1e3).format("0,0.000");
        ...
        checkboxInput();
        inputNumFormatting();
        calculatorStageFilling();
        falidateEvCalcForm();
        function falidateEvCalcForm() {
            const compareTypes = document.querySelectorAll("[data-compare-type]");
            if (compareTypes.length) compareTypes.forEach((compType => {
                const mpg = compType.querySelector('[data-ev-value="mpg"]');
                const vehiclePurchase = compType.querySelector('[data-ev-value="purchase-price"]');
                const yearsOwnsh = compType.querySelector('[data-ev-value="year-ownership"]');
                const fuelConst = compType.querySelector('[data-ev-value="fuel-cost"]');
                const inputs = [ mpg, vehiclePurchase, yearsOwnsh ];
                const yearlyCostLabel = compType.querySelector('[data-ev-label="yearly-cost"]');
                const costPerMile = compType.querySelector('[data-ev-label="cost-p-mile"]');
                const vehicTotCost = compType.querySelector('[data-ev-label="vehic-t-cost"]');
                if (mpg && vehiclePurchase && yearsOwnsh && fuelConst && yearlyCostLabel && costPerMile && vehicTotCost) inputs.forEach((input => {
                    input.addEventListener("input", (e => {
                        if (mpg.value !== "") {
                            yearlyCostLabel.style.display = "none";
                            costPerMile.style.display = "none";
                            console.log(`MPG Value: ${mpg.value}`);
                        } else {
                            yearlyCostLabel.removeAttribute("style");
                            costPerMile.removeAttribute("style");
                        }
                        if (yearsOwnsh.value !== "" && vehiclePurchase.value !== "") {
                            vehicTotCost.style.display = "none";
                            console.log(`Years of ownership: ${yearsOwnsh.value}, Vehicle Purchase Price: ${vehiclePurchase.value}, Cost of fuel: ${fuelConst.value}`);
                        } else vehicTotCost.removeAttribute("style");
                    }));
                }));
            }));
        }
