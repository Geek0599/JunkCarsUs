    ...
    (() => {
        ...
        function menuInit() {
            if (document.querySelector(".icon-menu")) document.addEventListener("click", (function(e) {
                if (bodyLockStatus && e.target.closest(".icon-menu")) {
                    bodyLockToggle();
                    !document.documentElement.classList.contains("menu-open") && document.documentElement.scrollTop > 0 ? window.scrollTo({
                        top: 0,
                        behavior: "smooth"
                    }) : null;
        ...
        function showSubMenu() {
            ...
            function matchMediaQuery(mediaQuery) {
                if (mediaQuery.matches) subMenus.forEach((menu => menu.setAttribute("hidden", ""))); else setTimeout((() => {
        ...
        function formValidate() {
            ...
            function checkInput(input) {
                if (input.required) {
                    ...
                    if (input.hasAttribute("data-minlength") && input.value.length < input.dataset.minlength) isError = addError(input);
                    if (input.hasAttribute("data-maxlenght") && input.value.length > input.dataset.maxlenght) isError = addError(input);
        ...
        function clickOnFileButton() {
            const fileBtn = document.querySelector("[data-file-input]");
            if (fileBtn) {
                const label = document.querySelector(`label[data-file-label][for="${fileBtn.id}"]`);
                if (label) label.addEventListener("keydown", (e => {
                    if (e.key === "Enter") fileBtn.click();
                }));
            }
        }
        ...
        function positionSubMenu() {
            ...
            if (referenceLinkForSubMenu.length) referenceLinkForSubMenu.forEach((link => {
                ...
                if (link && subMenu) popper_createPopper(link, subMenu, {
                    placement: "bottom",
                    modifiers: [ {
                        name: "offset",
                        options: {
